...
const ChatListItem = ({ chat, active, onClick, onContextMenu, onAvatarClick }) => { const longPressTimer = useRef(); const clearLongPressTimer = () => clearTimeout(longPressTimer.current); const handleInteractionStart = (e) => { e.preventDefault(); longPressTimer.current = setTimeout(() => { const event = e.touches ? e.touches[0] : e; onContextMenu(event, chat); }, 500); }; return ( <motion.div variants={listItemVariants} layout onClick={() => onClick(chat)} onContextMenu={(e) => { e.preventDefault(); onContextMenu(e, chat); }} onMouseDown={handleInteractionStart} onMouseUp={clearLongPressTimer} onMouseLeave={clearLongPressTimer} onTouchStart={handleInteractionStart} onTouchEnd={clearLongPressTimer} className={`flex items-center p-3 cursor-pointer rounded-xl transition-all duration-300 relative select-none ${active ? 'bg-white shadow-md' : 'hover:bg-gray-100'}`} > {active && <motion.div layoutId="activeChatBubble" className="absolute inset-0 bg-blue-500 rounded-xl z-0"></motion.div>} <div className="relative z-10 flex items-center w-full"> <div onClick={(e) => onAvatarClick(e, chat)} className="relative z-20 cursor-pointer" > <Avatar contact={chat} /> </div> <div className="flex-1 overflow-hidden ml-4"> <div className="flex justify-between items-center"> <p className={`font-bold truncate ${active ? 'text-white' : 'text-gray-800'}`}>{chat.name}</p> <span className={`text-xs flex-shrink-0 ${active ? 'text-blue-200' : 'text-gray-400'}`}>{chat.timestamp}</span> </div> <div className="flex justify-between items-start mt-1"> <p className={`text-sm truncate ${active ? 'text-blue-100' : 'text-gray-500'}`}>{chat.lastMessage}</p> {chat.unread > 0 && (<motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="bg-green-500 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">{chat.unread}</motion.span>)} </div> </div> </div> </motion.div> ); };
const MessageBubble = ({ msg, onContextMenu }) => { const isMe = msg.sender === 'me'; const longPressTimer = useRef(); const handleMouseDown = (e) => { if (e.button === 0) { longPressTimer.current = setTimeout(() => { onContextMenu(e, msg.id); }, 500); } }; const handleMouseUp = () => clearTimeout(longPressTimer.current); const handleTouchStart = (e) => { longPressTimer.current = setTimeout(() => { onContextMenu(e.touches[0], msg.id); }, 500); }; const handleTouchEnd = () => clearTimeout(longPressTimer.current); return ( <motion.div variants={listItemVariants} className={`flex w-full ${isMe ? 'justify-end' : 'justify-start'}`} > <motion.div layout className={`flex items-end gap-2 group max-w-xl ${isMe ? 'flex-row-reverse' : ''}`} onContextMenu={(e) => onContextMenu(e, msg.id)} onMouseDown={handleMouseDown} onMouseUp={handleMouseUp} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} > <div className={`p-3 px-4 rounded-t-2xl cursor-pointer ${ isMe ? 'bg-gradient-to-br from-blue-500 to-blue-600 text-white rounded-l-2xl' : 'bg-white text-gray-800 rounded-r-2xl shadow-sm' }`} > <p className="text-base">{msg.text}</p> <span className={`text-xs mt-1 block text-right ${isMe ? 'text-blue-200' : 'text-gray-400'}`}> {msg.timestamp} </span> </div> </motion.div> </motion.div> ); };
...
const InfoPanel = ({ chat, onClose }) => ( <motion.div initial={{x: '100%'}} animate={{x: 0}} exit={{x: '100%'}} transition={{type: 'spring', stiffness: 300, damping: 30}} className="w-[320px] bg-white border-l border-slate-200 flex flex-col flex-shrink-0"> <div className="p-4 border-b flex-shrink-0 text-center flex flex-col items-center"> <Avatar contact={chat} size="lg" /> <h3 className="text-xl font-bold mt-3">{chat.name}</h3> <p className="text-sm text-gray-500">{chat.isGroup ? `${chat.members} Anggota` : (chat.online ? 'Online' : 'Offline')}</p> </div> <div className="flex-1 overflow-y-auto p-4 custom-scrollbar space-y-6"> {chat.description && <div><h4 className="font-semibold text-gray-600 mb-2">Deskripsi</h4><p className="text-sm text-gray-800">{chat.description}</p></div>} <div><h4 className="font-semibold text-gray-600 mb-2">Media, File & Tautan</h4><div className="grid grid-cols-3 gap-2"><div className="aspect-square bg-gray-200 rounded-lg flex items-center justify-center"><FiImage className="text-gray-400"/></div><div className="aspect-square bg-gray-200 rounded-lg flex items-center justify-center"><FiFileText className="text-gray-400"/></div><div className="aspect-square bg-gray-200 rounded-lg flex items-center justify-center text-gray-400 text-xs font-bold">12+</div></div></div> {chat.isGroup && <div><h4 className="font-semibold text-gray-600 mb-2">Anggota ({chat.members})</h4><div className="space-y-2"><div className="flex items-center gap-3"><Avatar contact={dummyContacts[0]} size="sm"/><span>Alya Putri</span></div><div className="flex items-center gap-3"><Avatar contact={dummyContacts[2]} size="sm"/><span>Budi Santoso (Admin)</span></div></div></div>} <div className="space-y-1 pt-4 border-t"><MenuItem icon={FiBellOff} text="Bisukan Notifikasi" /><MenuItem icon={FiArchive} text="Arsipkan Obrolan" /><MenuItem icon={FiTrash2} text="Hapus Obrolan" /></div> </div> </motion.div> );
...
const KomunitasPage = () => {
    ...
    return (
        <>
            <style>{`.custom-scrollbar::-webkit-scrollbar{width:6px}.custom-scrollbar::-webkit-scrollbar-track{background:transparent}.custom-scrollbar::-webkit-scrollbar-thumb{background-color:#cbd5e1;border-radius:20px}.custom-scrollbar:hover::-webkit-scrollbar-thumb{background-color:#94a3b8}.input-chat:focus-within{box-shadow:0 0 0 2px #3b82f6}.custom-scrollbar-horizontal::-webkit-scrollbar{height:6px}.custom-scrollbar-horizontal::-webkit-scrollbar-track{background:transparent}.custom-scrollbar-horizontal::-webkit-scrollbar-thumb{background-color:#e2e8f0;border-radius:20px}.custom-scrollbar-horizontal:hover::-webkit-scrollbar-thumb{background-color:#cbd5e1}`}</style>
            ...
            <div className="w-full h-full flex font-sans overflow-hidden">
                ...
                <main className="flex-1 bg-slate-100 flex flex-col min-w-0">
                    {activeChat ? (
                        <>
                            <div className="flex-shrink-0 flex items-center p-4 border-b bg-white/70 backdrop-blur-sm shadow-sm z-10">
                                <div 
                                    className="flex items-center gap-4 flex-1 min-w-0 cursor-pointer"
                                    onClick={() => setViewedProfile(activeChat)}
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-sm text-green-500">
                                            {activeChat.online ? 'Online' : (activeChat.isGroup ? `${activeChat.members} anggota` : 'Offline')}
                                        </p>
                                    </div>
                                </div>
                                {/* --- [MODIFIKASI] Tombol Info dihapus, Call & Video dikembalikan --- */}
                            ...
                            <motion.div variants={listContainerVariants} initial="hidden" animate="visible" className="flex-1 overflow-y-auto p-6 space-y-2 custom-scrollbar"> 
                                <AnimatePresence>
                                    {messages.map(msg => 
                                        ...
                                        : <MessageBubble key={msg.id} msg={msg} onContextMenu={showMessageContextMenu} />
                                    )}
                                </AnimatePresence>
