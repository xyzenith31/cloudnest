...
const MessageBubble = ({ msg }) => { const isMe = msg.sender === 'me'; return (<motion.div variants={listItemVariants} className={`flex items-end gap-2 group ${isMe ? 'flex-row-reverse' : ''}`}><motion.div initial={{opacity: 0, y: 5}} whileHover={{opacity: 1, y: 0}} className={`hidden group-hover:flex items-center gap-1 p-1 bg-white rounded-full shadow-md border ${isMe ? 'mr-1' : 'ml-1'}`}><button className="p-1.5 rounded-full hover:bg-gray-200 transition-colors"><FiSmile size={16}/></button><button className="p-1.5 rounded-full hover:bg-gray-200 transition-colors"><FiThumbsUp size={16}/></button></motion.div><motion.div layout className={`max-w-md p-3 px-4 rounded-t-2xl ${isMe ? 'bg-gradient-to-br from-blue-500 to-blue-600 text-white rounded-l-2xl' : 'bg-white text-gray-800 rounded-r-2xl shadow-sm'}`}><p className="text-base">{msg.text}</p><span className={`text-xs mt-1 block text-right ${isMe ? 'text-blue-200' : 'text-gray-400'}`}>{msg.timestamp}</span></motion.div></motion.div>);};
...
const KomunitasPage = () => {
    ...
    const [isInfoPanelOpen, setIsInfoPanelOpen] = useState(false); // State untuk panel info
        ...
        <>
            ...
            <div className="w-full h-full flex font-sans overflow-hidden">
                ...
                <main className="flex-1 bg-slate-100 flex flex-col min-w-0">
                    {activeChat ? ( <> <div className="flex-shrink-0 flex items-center p-4 border-b bg-white/70 backdrop-blur-sm shadow-sm z-10"> <Avatar contact={activeChat} size="lg" /> <div className="ml-4 flex-1 min-w-0"><h2 className="text-lg font-bold text-gray-900 truncate">{activeChat.name}</h2><p className="text-sm text-green-500">{activeChat.online ? 'Online' : (activeChat.isGroup ? `${activeChat.members} anggota` : 'Offline')}</p></div> <div className="ml-auto flex items-center space-x-2"><button className="p-2 rounded-full hover:bg-gray-200 text-gray-500"><FiPhone /></button><button className="p-2 rounded-full hover:bg-gray-200 text-gray-500"><FiVideo /></button><button onClick={()=> setIsInfoPanelOpen(!isInfoPanelOpen)} className={`p-2 rounded-full text-gray-500 transition-colors ${isInfoPanelOpen ? 'bg-blue-100 text-blue-600' : 'hover:bg-gray-200'}`}><FiInfo /></button></div> </div> <motion.div variants={listContainerVariants} initial="hidden" animate="visible" className="flex-1 overflow-y-auto p-6 space-y-2 custom-scrollbar"> <AnimatePresence>{messages.map(msg => msg.type === 'date' ? <DateSeparator key={msg.id} text={msg.text}/> : <MessageBubble key={msg.id} msg={msg} />)}</AnimatePresence>{isTyping && <TypingIndicator/>}<div ref={messagesEndRef} /> </motion.div> <div className="flex-shrink-0 p-4 bg-white mt-auto"> <form onSubmit={handleSendMessage} className="flex items-center space-x-3 bg-gray-100 rounded-full p-2 transition-shadow duration-300 input-chat"> <button type="button" className="p-2 rounded-full hover:bg-gray-200 text-gray-500"><FiSmile size={22}/></button> <button type="button" className="p-2 rounded-full hover:bg-gray-200 text-gray-500"><FiPaperclip size={22}/></button> <input type="text" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder="Ketik pesan di sini..." className="flex-1 px-2 py-2 bg-transparent focus:outline-none" /> <motion.button whileTap={{ scale: 0.9 }} whileHover={{scale: 1.1}} type="submit" className="p-3 bg-blue-500 text-white rounded-full shadow-lg hover:bg-blue-600 transition-colors"><FiSend size={20}/></motion.button> </form> </div> </> ) : ( <div className="flex-1 flex flex-col items-center justify-center text-center text-gray-500"><FiMessageSquare size={64} /><h2 className="text-2xl font-semibold mt-4">Pilih Obrolan</h2><p>Mulai percakapan dengan memilih kontak dari daftar.</p></div> )}
                </main>
                
                {/* [FITUR BARU] Panel Info Kontak/Grup */}
                <AnimatePresence>
