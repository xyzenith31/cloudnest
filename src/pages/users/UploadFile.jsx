...
import {
    ...
    FaFilePowerpoint, FaFileExcel, FaWindows, FaApple, FaLinux // [PENAMBAHAN] Ikon FaLinux
} from 'react-icons/fa';
...
const getFileIcon = (fileType = '', fileName = '') => {
    ...
    // Prioritaskan ekstensi untuk ikon yang lebih spesifik
    switch (extension) {
        // Dokumen
        ...
        case '.txt':
        case '.rtf':
        case '.csv': return <FiFileText className="text-gray-500" />;
        ...
        case '.tar':
        case '.gz':
        case '.iso': return <FaFileArchive className="text-yellow-600" />;

        // Aplikasi (Installer)
        case '.apk': return <FaAndroid className="text-green-500" />;
        case '.exe':
        case '.msi': return <FaWindows className="text-sky-500" />;
        case '.dmg': return <FaApple className="text-gray-600" />;
        case '.ipa': return <FaApple className="text-blue-400" />; // Ikon Apple untuk iOS
        case '.deb': return <FaLinux className="text-orange-400" />; // Ikon Linux
        ...
        case '.ogg':
        case '.m4a':
        case '.flac': return <FiMusic className="text-pink-500" />;
    }

    // Fallback menggunakan tipe MIME jika ekstensi tidak cocok
    ...
    if (fileType.startsWith('audio/')) return <FiMusic className="text-pink-500" />;
    ...
    return (
        ...
        >
            ...
            <div className="file-item-details">
                ...
                {file.status === 'uploading' && (
                    <div className="progress-bar-container">
                        <motion.div className="progress-bar-fill" initial={{ width: 0 }} animate={{ width: `${file.progress || 0}%` }} />
                    </div>
                )}
            ...
            <div className="file-item-status">
                {file.status === 'pending' ? (
                    <motion.button whileTap={{ scale: 0.9 }} onClick={onRemove} className="p-1 rounded-full hover:bg-gray-100 text-gray-500">
                        <FiX />
                    </motion.button>
    ...
    return (
        <div className="info-card">
            <div className="section-header">
                <h2 className="section-title"><FiHardDrive />Pusat Penyimpanan</h2>
            </div>
            <p className="text-center text-3xl font-bold text-gray-800">
                {formatBytes(usedStorage, 1)} <span className="text-lg text-gray-500"> / 10 GB</span>
            </p>
...
const FilesSection = ({ title, icon: Icon, files, emptyText, children, onRemove }) => (
    <div className="info-card h-full flex flex-col">
        ...
        <div className="file-list-container">
            <AnimatePresence>
                {files.map(file => (
                    <FileItem key={file.id || file._id} file={file} onRemove={() => onRemove && onRemove(file.id)} />
                ))}
            </AnimatePresence>
            {files.length === 0 && (
                <div className="empty-list-placeholder"><p>{emptyText}</p></div>
            )}
        </div>
        {children}
    </div>
...
const UploadFile = () => {
    ...
    const fetchInitialData = useCallback(async () => {
        try {
            ...
            setNotification({ message: "Gagal memuat data file.", type: "error" });
        }
    }, []);
    ...
    const onDrop = useCallback(acceptedFiles => {
        ...
        const newFiles = acceptedFiles.map((file, i) => Object.assign(file, {
    ...
    const handleUpload = async () => {
        ...
        const uploadPromises = filesToUpload.map(file => {
            ...
            return uploadFile(formData, (e) => {
                ...
                setUploadingFiles(prev => prev.map(f => f.id === file.id ? { ...f, status: 'completed' } : f));
        ...
        try {
            ...
            setNotification({ message: `${uploadedFilesData.length} file berhasil diunggah!`, type: 'success' });
            setSessionHistory(prev => [...uploadedFilesData.map(f => ({ ...f, status: 'completed' })), ...prev]);
        } catch (error) {
            setNotification({ message: 'Beberapa file mungkin gagal diunggah.', type: 'error' });
        } finally {
    ...
    return (
        <div className="p-8">
            <style>{`
                ...
                .column-wrapper { display: flex; flex-direction: column; gap: 2rem; }
                .top-section { flex-grow: 1; display: flex; flex-direction: column; }
                .dropzone-reimagined-new { background-color: white; border: 2px dashed #e2e8f0; border-radius: 16px; padding: 2rem; cursor: pointer; transition: all 0.3s ease-in-out; flex-grow: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; }
                ...
                .info-card { background-color: white; border: 1px solid #e2e8f0; border-radius: 16px; padding: 1.5rem; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05); }
                ...
                .file-list-container { flex-grow: 1; min-height: 200px; overflow-y: auto; padding-right: 0.5rem; display: flex; flex-direction: column; gap: 0.75rem;}
                .empty-list-placeholder { flex-grow: 1; display: flex; align-items: center; justify-content: center; color: #9ca3af; text-align: center; }
                ...
                .animated-link { font-weight: 500; background-clip: text; -webkit-background-clip: text; color: transparent; background-image: linear-gradient(to right, #3b82f6, #60a5fa); position: relative; display: inline-block; transition: all 0.3s ease-in-out; }
                .animated-link::after { content: ''; position: absolute; width: 100%; height: 2px; bottom: 0; left: 0; background-color: #60a5fa; transform-origin: bottom right; transform: scaleX(0); transition: transform 0.3s ease-in-out; }
                .animated-link:hover::after { transform-origin: bottom left; transform: scaleX(1); }
                .card-footer-actions { margin-top: 1.5rem; display: flex; justify-content: flex-end; align-items: center; gap: 0.75rem; flex-shrink: 0; }
                .button-primary { background-color: #3b82f6; color: white; font-weight: 700; padding: 0.75rem 2rem; border-radius: 9999px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1), 0 2px 4px -2px rgba(0,0,0,0.1); }
                .button-primary:disabled { background-color: #9ca3af; cursor: not-allowed; }
                .button-danger { background-color: #ef4444; color: white; font-weight: 600; padding: 0.75rem 1.5rem; border-radius: 9999px; }
            ...
            <div className="upload-page-grid">
                <div className="column-wrapper">
                    ...
                    <FilesSection title="Riwayat Upload" icon={FiClock} files={sessionHistory} emptyText="Riwayat unggahan sesi ini kosong." />
                </div>
