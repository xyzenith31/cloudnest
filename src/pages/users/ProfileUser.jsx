...
import {
  ...
  FiCheck, FiSave, FiEye, FiEyeOff, FiCalendar, FiMonitor,
  FiLogOut, FiShield, FiX
} from 'react-icons/fi';
...
// --- [BARU] KOMPONEN POP-UP NOTIFIKASI LOKAL ---
      ...
      {message && (
        ...
        >
          <motion.div
            ...
            className="relative bg-white w-full max-w-sm p-8 rounded-2xl shadow-2xl text-center flex flex-col items-center"
            role="alert"
          >
            <button onClick={onClose} className="absolute top-3 right-3 text-gray-400 hover:text-gray-600 p-1 rounded-full">
              <FiX size={20} />
            </button>
...
const ProfileUser = () => {
    ...
    return (
        <>
            <PopupNotification message={notification.message} type={notification.type} onClose={() => setNotification({ message: '', type: '' })} />
            
            <div className="pt-8">
                <motion.div variants={itemVariants} initial="initial" animate="in" className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-8">
                    <div>
                ...
                <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch">
                    <div className="lg:col-span-1 flex flex-col space-y-8">
                        <motion.div variants={itemVariants} initial="initial" animate="in" className="h-full">
                            <div className="bg-white p-6 rounded-2xl shadow-lg border text-center h-full flex flex-col">
                                <div className="relative w-32 h-32 mx-auto group">
                                    ...
                                    />
                                    <label onClick={() => avatarInputRef.current.click()} className="absolute inset-0 flex items-center justify-center bg-black/50 rounded-full opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer">
                                ...
                                <div className="flex justify-center gap-3 mt-4">
                                    <button onClick={() => avatarInputRef.current.click()} className="px-4 py-2 text-sm font-semibold bg-sky-100 text-sky-800 rounded-lg cursor-pointer hover:bg-sky-200 flex items-center gap-2"> <FiUpload /> Ubah Foto </button>
                                    {avatarPreview && ( <button onClick={handleDeleteAvatar} className="px-4 py-2 text-sm font-semibold bg-red-100 text-red-800 rounded-lg hover:bg-red-200 flex items-center gap-2"> <FiTrash2 /> Hapus </button> )}
                                </div>
                        ...
                        <motion.div variants={itemVariants} initial="initial" animate="in">
                            <div className="bg-white p-6 rounded-2xl shadow-lg border">
                                <h3 className="text-xl font-bold text-gray-800 mb-4">Perangkat Terhubung</h3>
                                {devices.find(d => d.current) && (
                                    <p className="text-sm text-gray-500 mb-4"> Saat ini aktif di <span className="font-semibold text-gray-700">{devices.find(d => d.current).device}</span>.</p>
                                )}
                    ...
                    <motion.form id="profile-form" onSubmit={handleSubmit} variants={itemVariants} initial="initial" animate="in" className="lg:col-span-2 flex flex-col space-y-8">
                        <div className="bg-white p-8 rounded-2xl shadow-lg border h-full">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <CustomSelect options={genderOptions} selected={selectedGender} onChange={handleGenderChange} />
                                </div>
                            </div>
                        </div>
                        <div className="bg-white p-8 rounded-2xl shadow-lg border h-full">
                            ...
                            <div className="grid grid-cols-1 gap-6">
                                <div className="relative">
                                    ...
                                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-[38px] text-gray-400 hover:text-gray-600">{showPassword ? <FiEye /> : <FiEyeOff />}</button>
                                </div>
                                <div className="relative">
                                    ...
                                    <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="text-gray-400 hover:text-gray-600">{showConfirmPassword ? <FiEye /> : <FiEyeOff />}</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                {deviceToRemove && (
                    <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                        <motion.div initial={{scale:0.9}} animate={{scale:1}} exit={{scale:0.9}} className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 text-center">
                            <div className="w-16 h-16 bg-red-100 rounded-full flex items-center justify-center mx-auto"><FiShield className="text-4xl text-red-500"/></div>
            ...
            <AnimatePresence>
                {isConfirmModalOpen && (
                    <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                        <motion.div initial={{scale:0.9}} animate={{scale:1}} exit={{scale:0.9}} className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 text-center">
                            <div className="w-16 h-16 bg-sky-100 rounded-full flex items-center justify-center mx-auto"><FiSave className="text-4xl text-sky-500"/></div>
